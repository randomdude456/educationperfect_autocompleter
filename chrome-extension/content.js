(function () {
  ...
  const STORAGE_KEY = 'ep-auto-answer-5.0-answer-map';
  const SETTINGS_KEY = 'ep-auto-answer-5.0-settings';
  ...
  async function loadExtensionState() {
    try {
      ...
      const storedMap = stored?.[STORAGE_KEY];
      const storedSettings = stored?.[SETTINGS_KEY];

      answerMap = storedMap && typeof storedMap === 'object' ? storedMap : {};
      settings = {
        ...DEFAULT_SETTINGS,
      ...
      diagnostics.push('[startup] Could not read saved extension data');
    }
  }
  ...
  function saveMap() {
    chrome.storage.local.set({ [STORAGE_KEY]: answerMap }).catch(() => {
      addDiagnostic('Could not save vocabulary');
    });
  }
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      <div id="ep-handle">
        <span id="ep-logo">EP Auto <span class="ep-version">5.0</span></span>
    ...
    panel.querySelector('#ep-clear-list').addEventListener('click', () => {
      ...
      chrome.storage.local.remove(STORAGE_KEY).catch(() => {
        addDiagnostic('Could not clear saved vocabulary');
      });
      readyToStart = false;
      armed = false;
      updateCount();
  ...
  function startObserver() {
    ...
    observer = new MutationObserver(() => {
      ...
      if (listRoot && isVisible(listRoot) && !loadInProgress && !readyToStart) {
        setState('Vocabulary list detected', 'Click Load vocabulary');
      }
    });
